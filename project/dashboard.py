...
def process_video(cap, limit_time=60):
    global frame_count, inference_times, detection_log
    frame_count, inference_times, detection_log = 0, [], []
    ...
    while cap.isOpened():
        ...
        for box, score, label in zip(boxes, scores, labels):
            detection_log.append(
                {
                    "시간": now,
                    "클래스": "fire" if label == 0 else "smoke",
                    "신뢰도": round(score, 2),
                    "좌표": f"({round(box[0], 2)}, {round(box[1], 2)}, {round(box[2], 2)}, {round(box[3], 2)})",
                }
            )
