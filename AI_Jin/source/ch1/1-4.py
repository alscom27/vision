...
# 경로 설정
img_tar_path = "images.tar.gz"
ann_tar_path = "annotations.tar.gz"
img_extract_path = "datasets/oxford_pets/images/images"
ann_extract_path = "datasets/oxford_pets/annotations/annotations"

# 다운로드 및 압축 해제 함수
def download_and_extract(url, tar_path, extract_path):
    if not os.path.exists(extract_path):
        print(f"{extract_path} 경로가 없으므로 다운로드 및 압축 해제 진행 중...")
        urllib.request.urlretrieve(url, tar_path)
        with tarfile.open(tar_path) as tar:
            tar.extractall(path=os.path.dirname(extract_path))
        print(f"{extract_path} 압축 해제 완료!")
    else:
        print(f"✔ 이미 존재: {extract_path} — 다운로드 및 압축 생략")
